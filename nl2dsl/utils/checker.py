...
class Checker:
    def __init__(
        ...
        self.errors = {"errors": [], "warnings": [], "info": []}
        self.dsl = dsl
        self.dsl_dict = {}
        ...
        if value["task_type"] == "print":
            if "goto" in value:
                if value["goto"] == state_name:
                    self.errors["warnings"].append(f"Upon task completion, {state_name} transitions to itself, will result in infinite loop")
        if value["task_type"] == "input":
            ...
            if "error_goto" in value:
                if value["error_goto"] == state_name:
                    self.errors["warnings"].append(f"Upon error in {state_name} task it transitions to itself, can result in infinite loop")
        if value["task_type"] == "plugin":
            if "transitions" in value:
                for transition in value["transitions"]:
                    if transition["goto"] == state_name:
                        self.errors["warnings"].append(
        ...
        if value["task_type"] == "condition":
            if "else_goto" in value:
                if value["else_goto"] == state_name:
                    self.errors["warnings"].append(f"Upon unhandled condition, {state_name} transitions to itself, will result in infinite loop")
                ...
                if task['task_type'] == 'plugin':
                    ...
                    for var in task['plugin']['inputs'].values():
                        if var not in written_variables:
                            self.errors["errors"].append(f"Variable {var} used in task {task_name} not found in dsl.")
        ...
        for key, value in self.variables_dict.items():
            if "type" in value:
                if value["type"] not in [
                    ...
                    self.errors["errors"].append(f"{key} type is not valid")
            else:
                self.errors["errors"].append(f"{key} type is missing")
